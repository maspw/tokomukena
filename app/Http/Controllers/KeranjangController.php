...
class KeranjangController extends Controller
{
    ...
    public function daftarbarang()
    {
        ...
        $pembeli = Pembeli::where('user_id', $id_user)
                ...
                ->first();
        if (!$pembeli) {
            return redirect()->back()->with('error', 'Data pembeli tidak ditemukan. Hubungi administrator.');
        }
    ...
    public function tambahKeranjang(Request $request)
    {
        ...
        try {
            ...
            $pembeli = Pembeli::where('user_id', $id_user)
                            ...
                            ->first();
            if (!$pembeli) {
                return response()->json(['success' => false, 'message' => 'Data pembeli tidak ditemukan. Hubungi administrator.']);
            }
            $id_pembeli = $pembeli->id;
    ...
    public function lihatkeranjang(){
        ...
        $barang = DB::table('penjualan')
                        ->join('penjualan_barangs', 'penjualan.id', '=', 'penjualan_barangs.penjualan_id')
                        ->join('pembayaran', 'penjualan.id', '=', 'pembayaran.penjualan_id')
                        ->join('barang', 'penjualan_barangs.barang_id', '=', 'barang.id')
                        ->join('pembeli', 'penjualan.pembeli_id', '=', 'pembeli.id')
                        ->select('penjualan.id','penjualan.no_faktur','pembeli.nama_pembeli', 'penjualan_barangs.barang_id', 'barang.nama_barang','penjualan_barangs.harga_jual', 
                                 'barang.foto','pembayaran.order_id',
                                  DB::raw('SUM(penjualan_barangs.jml) as total_barang'),
                                  DB::raw('SUM(penjualan_barangs.harga_jual * penjualan_barangs.jml) as total_belanja'))
                        ->where('penjualan.pembeli_id', '=',$id_pembeli) 
                        ->where(function($query) {
                            $query->where('pembayaran.gross_amount', 0)
                                  ->orWhere(function($q) {
                        ...
                        ->groupBy('penjualan.id','penjualan.no_faktur','pembeli.nama_pembeli','penjualan_barangs.barang_id', 'barang.nama_barang','penjualan_barangs.harga_jual',
                                  'barang.foto','pembayaran.order_id',
                                 )
                        ->get();
    ...
    public function hapus($barang_id)
    {
        ...
        $pembeli = Pembeli::where('user_id', $id_user)
                        ...
                        ->first();
        if (!$pembeli) {
            return response()->json(['success' => false, 'message' => 'Data pembeli tidak ditemukan.']);
        }
        $id_pembeli = $pembeli->id;
    ...
    public function lihatriwayat(){
        ...
        $detail_barang = DB::table('penjualan_barangs')
            ->join('barang', 'penjualan_barangs.barang_id', '=', 'barang.id')
